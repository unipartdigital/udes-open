...
class ByCarrier(SuggestLocationPolicy):
    """Find locations based on other quants for the same carrier"""
    ...
    def get_values_from_mls(self, mls):
        ...
        """
        return {
            "carrier": mls.move_id.sale_line_id.order_id.u_carrier_id.ensure_one(),
    ...
    def get_values_from_dict(self, values):
        ...
        carrier = picking.move_id.sale_line_id.order_id.u_carrier_id
        return {
            "carrier": carrier,
            "location": picking.location_dest_id,
        }

    def get_locations(self, carrier, location, **kwargs):
        """
        Return stock.locations whose current quants are derived from a sales order
        for the same carrier as the one passed in.
