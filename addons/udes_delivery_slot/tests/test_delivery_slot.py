...
class TestDeliverySlot(common.SavepointCase):
    ...
    def test01_constraint(self):
        ...
        with self.assertRaises(IntegrityError), mute_logger('odoo.sql_db'):
            Slot.create({'identifier': '001', 'start_time': -1.0, 'end_time': 10.5})

    def test02_constraint(self):
        """Check constraints are enforced"""
        Slot = self.env['udes.delivery.slot']
        with self.assertRaises(IntegrityError), mute_logger('odoo.sql_db'):
            Slot.create({'identifier': '002', 'start_time': 10.0, 'end_time': 1.0})

    def test03_constraint(self):
        """Check constraints are enforced"""
        Slot = self.env['udes.delivery.slot']
        with self.assertRaises(IntegrityError), mute_logger('odoo.sql_db'):
            Slot.create({'identifier': '003', 'start_time': 12.0, 'end_time': 24.3})

    def test04_constraint(self):
        """Check constraints are enforced"""
        Slot = self.env['udes.delivery.slot']
        with self.assertRaises(IntegrityError), mute_logger('odoo.sql_db'):
            Slot.create({'identifier': '004', 'start_time': 24.0, 'end_time': 36.5})

    def test05_as_time(self):
        """Check the as_time() method returns correct times"""
        Slot = self.env['udes.delivery.slot']
        slot1 = Slot.create({'identifier': '001', 'start_time': 09.50, 'end_time': 13.00})
        slot2 = Slot.create({'identifier': '002', 'start_time': 0.00, 'end_time': 24.00})
