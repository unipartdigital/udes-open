...
class SecureAction(Action):
    ...
    def load(self, action_id, additional_context=None):
        ...
        group_trusted_user_id = request.env.ref("udes_security.group_trusted_user").id
        ...
        if not self._user_has_access_right(
            action_group_ids, menu_path, user_group_ids, group_no_one_id, group_trusted_user_id
        ):
            _logger.warning(
    ...
    def _get_object_group_ids(self, obj_id, relation_table, inverse_name):
        ...
        request.env.cr.execute(query, (obj_id,))
    ...
    def _get_menu_info_from_action_id(self, action_id):
        ...
        query = """
            SELECT
                menu.parent_path AS parent_path,
        ...
        result = request.env.cr.fetchone()
        if result:
            (parent_path, relation_table, inverse, action_id) = result
            # Get ids in order from parent --> child of the menus
            parent_path_ids = [int(str_menu_id) for str_menu_id in parent_path.split("/")[:-1]]
    ...
    def _user_has_access_right(
        self, action_group_ids, parent_path, user_group_ids, group_no_one_id, group_trusted_user_id
    ):
        """
        ...
        :param: group_trusted_user_id: int() id for Other Extra Rights/Trusted User group
        ...
        # If Technical Features is a group we need for access, then it implies Trusted User
        ...
        if group_no_one_id in group_ids_for_access:
            if group_trusted_user_id not in user_group_ids:
