...
class TestTargetStorageTypes(common.BaseUDES):

    @classmethod
    def setUpClass(cls):
        super(TestTargetStorageTypes, cls).setUpClass()
    ...
    def test03_target_storage_format_pallet_products(self):
        """ Test for basic usage of target_storage_format pallet_products"""
    ...
    def test04_target_storage_format_pallet_packages(self):
        """ Test for basic usage of target_storage_format pallet_packages"""
        Package = self.env['stock.quant.package']
        package = Package.get_package('test_package', create=True)
        create_info = [{'product': self.apple, 'qty': 4}]
        picking = self.create_picking(self.picking_type_in,
    ...
    def test07_target_storage_format_package_over_receive_pallet_products(self):
        """Tests over receiving for target_storage_format pallet_products"""
        Package = self.env['stock.quant.package']
        package = Package.get_package('test_package', create=True)
        create_info = [{'product': self.apple, 'qty': 2}]
        picking = self.create_picking(self.picking_type_in,
    ...
    def test08_target_storage_format_package_over_receive_pallet_packages(self):
        """Tests over receiving for target_storage_format pallet_packages"""
        Package = self.env['stock.quant.package']
        package = Package.get_package('test_package', create=True)
        create_info = [{'product': self.apple, 'qty': 2}]
        picking = self.create_picking(self.picking_type_in,
    ...
    def test13_target_storage_format_mixed_product(self):
        """Tests receiving mixed packages for target_storage_format product"""
        create_info = [{'product': self.apple, 'qty': 4},
    ...
    def test14_target_storage_format_mixed_package(self):
        """Tests receiving mixed packages for target_storage_format package"""
        Package = self.env['stock.quant.package']
        package = Package.get_package('test_package', create=True)
        create_info = [{'product': self.apple, 'qty': 4},
                       {'product': self.banana, 'qty': 4}]
    ...
    def test15_target_storage_format_mixed_pallet_products(self):
        """Tests receiving mixed packages for target_storage_format pallet_products"""
        Package = self.env['stock.quant.package']
        package = Package.get_package('test_package', create=True)
        create_info = [{'product': self.apple, 'qty': 4},
                       {'product': self.banana, 'qty': 4}]
    ...
    def test16_target_storage_format_mixed_pallet_packages(self):
        """Tests receiving mixed packages for target_storage_format pallet_packages"""
        Package = self.env['stock.quant.package']
        package = Package.get_package('test_package', create=True)
        create_info = [{'product': self.apple, 'qty': 4},
                       {'product': self.banana, 'qty': 4}]
    ...
    def test17_target_storage_format_multiple_packages_from_same_product_package(self):
        """Tests that mutiple sets of the same product can be made from a single picking
    ...
    def test18_target_storage_format_multiple_packages_from_same_product_pallet_products(self):
        ...
        apple_lines_done2 = picking.move_line_ids.filtered(lambda x: x.product_id == self.apple and x.qty_done > 0)
        apple_lines_done2 -= apple_lines_done
        self.validate_move_lines(apple_lines_done2, self.apple, 2, 2, is_pallet=True, package_name=package2.name)

    def test19_target_storage_format_multiple_packages_from_same_product_pallet_packages(self):
        """Tests that mutiple sets of the two products can be made from a single picking
        ...
        apple_lines_done2 = picking.move_line_ids.filtered(lambda x: x.product_id == self.apple and x.qty_done > 0)
        apple_lines_done2 -= apple_lines_done
        self.validate_move_lines(apple_lines_done2, self.apple, 2, 2, is_pallet=True, is_package=True,
                                 package_name=package2.name)

    def test20_target_storage_format_multiple_mixed_packages_from_two_products_package(self):
        """Tests that mutiple sets of the two products can be made from a single picking
           for target_storage_format package.
        """
        Package = self.env['stock.quant.package']
        ...
        apple_lines_done = picking.move_line_ids.filtered(lambda x: x.product_id == self.apple)
        banana_lines_done = picking.move_line_ids.filtered(lambda x: x.product_id == self.banana)
        self.validate_move_lines(apple_lines_done, self.apple, 4, 0)
        self.validate_move_lines(banana_lines_done, self.banana, 4, 0)
        ...
        self.validate_move_lines(apple_lines_done2, self.apple, 2, 2, is_package=True, package_name=package2.name)
        self.validate_move_lines(banana_lines_done2, self.banana, 2, 2, is_package=True, package_name=package2.name)

    def test21_target_storage_format_multiple_mixed_packages_from_two_products_pallet_products(self):
        """Tests that mutiple sets of the two products can be made from a single picking
           for target_storage_format pallet_products.
        """
        Package = self.env['stock.quant.package']
        ...
        banana_lines_done2 -= banana_lines_done
        self.validate_move_lines(apple_lines_done2, self.apple, 2, 2, is_pallet=True, package_name=package2.name)
        self.validate_move_lines(banana_lines_done2, self.banana, 2, 2, is_pallet=True, package_name=package2.name)

    def test22_target_storage_format_multiple_mixed_packages_from_two_products_pallet_packages(self):
        """Tests that mutiple sets of the two products can be made from a single picking
           for target_storage_format pallet_packages.
        """
        Package = self.env['stock.quant.package']
        ...
        apple_lines_done2 -= apple_lines_done
        banana_lines_done2 -= banana_lines_done
        self.validate_move_lines(apple_lines_done2, self.apple, 2, 2,
                                 is_pallet=True, is_package=True, package_name=package2.name)
        self.validate_move_lines(banana_lines_done2, self.banana, 2, 2,
                                 is_pallet=True, is_package=True, package_name=package2.name)

    def test23_over_receive_while_picking_type_doesnt_allow_it(self):
        """ Checks that the correct error is thrown when over-receiving
            when it is not allowed by picking type.
        """
