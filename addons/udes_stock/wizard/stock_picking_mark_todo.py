...
class StockPickingMarkTodo(models.TransientModel):
    ...
    draft_picking_ids = fields.Many2many(
        "stock.picking", string="Draft Pickings", domain=[("state", "=", "draft")], relation="stock_picking_mark_todo_draft_rel",
    )
    ...
    non_draft_picking_ids = fields.Many2many(
        ...
        domain=[("state", "!=", "draft")],
        readonly=True,
        relation="stock_picking_mark_todo_non_draft_rel",
    )
    ...
    def default_get(self, fields):
        ...
        res.update(
            {
                "draft_picking_ids": [(6, 0, draft_picking_ids)],
                "non_draft_picking_ids": [(6, 0, non_draft_picking_ids)],
            }
        )
        return res
