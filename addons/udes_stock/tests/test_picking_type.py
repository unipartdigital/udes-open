...
class TestPickingType(common.BaseUDES):
    ...
    def test01_get_suggested_locations_exact_match(self):
        ...
        location = picking.get_suggested_locations(move_line_ids=mls)
    ...
    def test02_get_suggested_locations_by_product(self):
        ...
        locations = picking.get_suggested_locations(products=self.apple)
    ...
    def test03_get_suggested_locations_by_product_using_move_lines(self):
        ...
        mls = picking.move_line_ids
        locations = picking.get_suggested_locations(move_line_ids=mls)
        self.assertEqual(locations, self.test_locations)
    ...
    def test04_get_suggested_locations_by_package(self):
        ...
        locations = picking.get_suggested_locations(package=self.package_one)
        self.assertEqual(locations, self.test_locations)
    ...
    def test05_get_suggested_locations_wrong_move_lines(self):
        ...
        with self.assertRaises(ValidationError) as err:
            locations = picking2.get_suggested_locations(move_line_ids=mls)
    ...
    def test06_get_suggested_locations_by_products_missing_parameter(self):
        """ Validation error raised when trying to suggest locations of a
            picking by products and not passing any parameter.
        """
        self.picking_type_putaway.u_drop_location_policy = 'by_products'
        ...
        msg = 'Products missing to suggest location for.'
        self.assertEqual(err.exception.name, msg)

    def test07_get_suggested_locations_by_package_wrong_package(self):
        """ Validation error raised when trying to suggest locations for a
            package not present at the pikcing.
        """
        self.picking_type_putaway.u_drop_location_policy = 'by_packages'
        ...
        msg = 'Package %s not found in picking %s in order to suggest ' \
              'drop off locations for it.' %\
              (self.package_two.name, picking.name)
        self.assertEqual(err.exception.name, msg)
