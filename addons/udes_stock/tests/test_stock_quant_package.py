...
class TestStockQuantPackageModel(BaseUDES):
    ...
    def test11_mls_can_fulfill_success_no_excess_multiple_move_lines(self):
        ...
        pick = self.create_picking(self.picking_type_pick, products_info=product_info, assign=True, confirm=True, location_id=self.test_stock_location_02.id)
        # Get mls
    ...
    def test12_mls_can_fulfill_success_with_excess_simple(self):
        ...
        pick = self.create_picking(self.picking_type_pick, products_info=[{'product': self.apple, 'qty': 18}], assign=True, confirm=True, location_id=self.test_stock_location_02.id)
        # Get apple mls
        mls = pick.move_line_ids
    ...
    def test13_mls_can_fulfill_success_with_excess_multiple_move_lines(self):
        ...
        self.create_quant(self.fig.id, self.test_stock_location_01.id, 15, package_id=self.test_package.id)
        self.create_quant(self.apple.id, self.test_stock_location_02.id, 20)
        ...
        # Create pick
        product_info = [{'product': self.banana, 'qty': 17}, {'product': self.fig, 'qty': 15}, {'product': self.apple, 'qty': 18}]
        pick = self.create_picking(self.picking_type_pick, products_info=product_info, assign=True, confirm=True, location_id=self.test_stock_location_02.id)
        # Get apple mls
        mls = pick.move_line_ids
