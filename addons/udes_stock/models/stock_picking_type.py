...
class StockPickingType(models.Model):
    ...
    u_use_part_pallets = fields.Boolean(
        ...
        help=(
            "Overview: creates a backorder for stock that is partially picked."
            " This setting enables the progression of part of a picking, by creating"
            " backorders on pallet change. This allows picked parts to immediately"
            " become availble for subsequent picks in the route.\n"
            "Example: A large Goods In pick is received containing multiple products."
            " Due to lack of space in the warehouse the operatives can only process"
            " part of the Goods In pick at a time.\n"
            "With this setting enabled the warehouse operatives can pick 50 x"
            " product A and a backorder is created for the remaining products to be"
            " processed at another time.",
            "With this settings disabled the warehouse operatives would need"
            " to process all products on the pick before the subsequent picks in the"
            " route are ready.",
        ),
        default=False,
    )
