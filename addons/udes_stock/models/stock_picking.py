...
class StockPicking(models.Model):
    ...
    def _find_unreservable_moves(self, batch_size=1000, return_fast=False):
        ...
        for r, batch in self.batched(size=batch_size):
            ...
            batch.with_context(prefetch_fields=False).mapped("move_lines")
            batch.mapped("move_lines").read(
    ...
    def reserve_stock(self):
        ...
        recordset is not empty, it is the caller's responsibility to make sure
        ...
        for picking_type in picking_types:
            ...
            while reserve_all or to_reserve > 0:
                ...
                for picking in pickings_to_process:
                    ...
                    unreservable_moves = to_assign._find_unreservable_moves(return_fast=bool(self))
                    if unreservable_moves:
                        moves = to_assign.mapped("move_lines")
                        moves_todo = (
                    ...
                    while True:
                        ...
                        try:
                            with self.env.cr.savepoint():
                                to_assign = to_assign._reserve_stock_assign()
                                batch._compute_state()

                                processed |= to_assign

                                unsatisfied = to_assign.filtered(
                                    lambda x: x.state not in ["assigned", "cancel", "done"]
                                )
                                mls = to_assign.mapped("move_line_ids")
                                if unsatisfied:
                    ...
                    if to_reserve <= 0 and not reserve_all:
                        break
