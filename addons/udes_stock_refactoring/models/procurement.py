    ...
    def get_group(self, group_identifier, create=False):
        ...
        if isinstance(group_identifier, int):
            ...
            raise ValidationError(
                _("Unable to create domain for group search from" " identifier of type %s")
        ...
        if not results:
            if create and name:
                ...
                raise ValidationError(
                    _("Group not found for " "identifier %s") % str(group_identifier)
                )
        if len(results) > 1:
            raise ValidationError(
                _("Too many groups found for " "identifier %s") % str(group_identifier)
            )
        return results
