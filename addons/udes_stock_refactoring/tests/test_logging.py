...
class TestDefaultRefactoringWizard(TestRefactoringWizard):
    def setUp(self):
        ...
        self.pick_2 = self.create_picking(
            ...
            location_dest_id=self.test_goodsout_loceation_01.id,
        )
    ...
    def test_logging_wizard(self):
        ...
        with self.assertLogs("odoo.addons.udes_stock_refactoring.models.stock_move", level="INFO") as cm:
             move_refactoring_wizard.do_refactor()

        log_output = "\n".join(cm.output)

        self.assertIn(f"Original moves:", log_output)
        self.assertIn(f"New moves:", log_output)
        self.assertIn(f"Move Lines:", log_output)
...
class TestRefactorLogging(common.BaseUDES):
    def test_refactor_logging(self):
        ...
        original_move_ids = self.moves.mapped("move_line_ids").ids
        pickings = self.moves.mapped("picking_id")
        picking_info = [f"{pick.id}, {(pick.name)}" for pick in pickings]
        picking_type = "f{picking.picking_type_id}, {picking.picking_type_id.name}"
        ...
        self.assertIn(f"Original moves: {original_move_ids}", log_output)
        self.assertIn(f"New moves: {new_moves.ids}", log_output)
        self.assertIn(f"Move Lines: {original_move_ids}", log_output)

        for pick in picking_info:
            self.assertIn(pick, log_output)
        
        self.assertIn(f"Picking type: {picking_type}", log_output)
        self.assertIn(f"Refactored by method: {source}", log_output)
        self.assertRegex(log_output, r"Stage: \w+")
