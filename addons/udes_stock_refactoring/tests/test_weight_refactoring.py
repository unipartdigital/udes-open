...
class TestRefactoringAssignSplittingWeight(RefactoringByWeightBase):
    ...
    def test_refactor_assign_not_all_available_refactor(self):
        ...
        # TODO palabaster: This test fails here. The unreserved stock is not split to its own pick.
        self.assertEqual(len(all_pickings), 3)
        quantities_ordered_and_reserved = [
            (
                sum(pick.move_lines.mapped("product_uom_qty")),
    ...
    def test_refactor_assign_not_all_available_refactor_3(self):
        ...
        # TODO palabaster: This test fails later on. It only passes the length check because the unreserved stock exceeds 20kg.
        # The unreserved stock is not split to its own pick.
        self.assertEqual(len(all_pickings), 3)
        quantities_ordered_and_reserved = [
            (
                sum(pick.move_lines.mapped("product_uom_qty")),
        ...
        # TODO palabaster: This will raise but we don't care about the unassigned pick exceeding the weight, so we should filter all_pickings here..
        self.assert_weight_not_exceeded(all_pickings)
