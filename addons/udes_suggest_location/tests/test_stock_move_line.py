...
class CommonTestCase(common.SuggestedLocations):
    """Tests that don't depend on specific policy behaviours."""

    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.MoveLine = cls.env["stock.move.line"]
        # Set policies, use by_empty_location for pick
        cls.picking_type_pick.u_suggest_locations_policy = "by_empty_location"
        cls.ByProduct = SUGGEST_LOCATION_REGISTRY["by_empty_location"](cls.env)
        ...
        cls.pick_empty_locs = pick_empty_locs
    ...
    def test_get_policy_class(self):
        ...
        by_empty_location = self.MoveLine._get_policy_class(self.picking_type_pick)
        # Check naming
        self.assertEqual(by_empty_location.name(), "by_empty_location")
    ...
    def test_suggest_locations_errors_with_not_enough_infos(self):
        """Suggest locations by picking type should throw error when not enough information is
        given
        """
        suggestions = [
            {},
            {"picking": self.picking_pick},
        ...
        for suggestion in suggestions:
            with self.subTest(suggestion=suggestion):
                with self.assertRaises(ValueError) as e:
                    self.MoveLine.suggest_locations(**suggestion)
                self.assertEqual(
                    str(e.exception),
                    "Missing information to suggest locations, "
                    "please provide either move lines or picking and values!",
                )
    ...
    def test_suggest_locations_limit_results(self):
        ...
        self.assertEqual(self.apple_mls.suggest_locations(), self.pick_empty_locs)
        # Limit the results
    ...
    def test_set_invalid_dest_location(self):
        ...
        # Try to create a pick with an invalid destination location (that
        # is not empty).
...
class ByProductTestCase(common.SuggestedLocations):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.MoveLine = cls.env["stock.move.line"]
...
class ByOriginTestCase(common.SuggestedLocations):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.MoveLine = cls.env["stock.move.line"]
        ...
        for picking, location in zip(
            (cls.picking1, cls.picking3), (cls.test_check_location_01, cls.test_check_location_02)
        ):
        ...
        cls.damson_mls = cls.mls.filtered(lambda ml: ml.product_id == cls.damson)
    ...
    def test_validation_of_locations_by_product_non_enforced(self):
        ...
        self.assertEqual(self.banana_mls.suggest_locations(), self.test_check_location_01)
        self.assertEqual(self.cherry_mls.suggest_locations(), self.test_check_location_01)
        self.assertEqual(self.damson_mls.suggest_locations(), self.test_check_location_02)
